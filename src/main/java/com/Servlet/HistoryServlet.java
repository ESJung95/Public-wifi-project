            ...
            try {
                ...
                try {
                    ...
                    if (affectedRows > 0) {
                        ...
                        response.getWriter().write("데이터베이스에 위치 정보를 저장했습니다.");
                    } else {
                        // INSERT 실패 처리
                        response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "데이터베이스에 위치 정보를 저장하는 동안 오류가 발생했습니다.");
                    }
                ...
                e.printStackTrace();
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "잘못된 형식의 위치 정보입니다.");
            }
        } else {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "위치 정보가 제공되지 않았습니다.");
        }
    }
}